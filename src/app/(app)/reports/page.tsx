...
export default function ReportsPage() {
  ...
  const exportFactoriesExcel = () => {
    const rows = factoriesSummary.map((f: AnyData) => ({
      ...
      'إجمالي النقلات': f.totalTrips, 'نقداً': f.cashTrips, 'مسوّاة لاحقاً': f.laterTrips, 'ذمة': f.creditTrips,
              ...
              <Card>
                <CardHeader>
                  <h2 className="font-semibold text-slate-800 flex items-center gap-2">
                    <Users size={16} /> ترتيب المصانع ({factoriesSummary.filter((f: AnyData) => f.totalTrips > 0).length} مصنع نشط)
                  </h2>
                </CardHeader>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
